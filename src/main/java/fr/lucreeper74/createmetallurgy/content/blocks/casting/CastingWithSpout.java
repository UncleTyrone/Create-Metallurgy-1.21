...
public enum CastingWithSpout implements BlockSpoutingBehaviour {
    ...
    public int fillBlock(Level level, BlockPos pos, SpoutBlockEntity spout, FluidStack availableFluid,
        ...
        IFluidHandler fluidHandler = level.getCapability(Capabilities.FluidHandler.BLOCK, pos, Direction.UP);
        ...
        if (amount < 1000
                && fluidHandler.fill(availableFluid.copyWithAmount(amount + 1),
                        IFluidHandler.FluidAction.SIMULATE) > amount)
            return 0;

        // Return amount filled into the table/basin
        return fluidHandler.fill(availableFluid,
                simulate ? IFluidHandler.FluidAction.SIMULATE : IFluidHandler.FluidAction.EXECUTE);
    }

    public static void registerDefaults() {
        BlockSpoutingBehaviour.BY_BLOCK_ENTITY.register(CMBlockEntityTypes.CASTING_BASIN.get(),
                CastingWithSpout.INSTANCE);
        BlockSpoutingBehaviour.BY_BLOCK_ENTITY.register(CMBlockEntityTypes.CASTING_TABLE.get(),
                CastingWithSpout.INSTANCE);
    }
}