...
public class FoundryScenes {
        public static void foundryBasin(SceneBuilder builder, SceneBuildingUtil util) {
                ...
                // Show Faucet
                ElementLink<WorldSectionElement> depotLink = scene.world()
                                .showIndependentSection(util.select().position(depotPos), Direction.EAST);
                ...
                scene.world().modifyBlock(burnerPos,
                                s -> s.setValue(BlazeBurnerBlock.HEAT_LEVEL, BlazeBurnerBlock.HeatLevel.KINDLED),
                                false);
                scene.idle(40);
                ...
                scene.world().modifyBlockEntity(lidPos, FoundryLidBlockEntity.class,
                                FoundryLidBlockEntity::startProcessingBasin);
                scene.idle(40);
                ...
                scene.effects().emitParticles(util.vector().centerOf(drainPos),
                                scene.effects().simpleParticleEmitter(FluidFX.getFluidParticle(gold), Vec3.ZERO), 3,
                                20);
                scene.world().modifyBlockEntity(drainPos, ItemDrainBlockEntity.class, be -> {
                        be.getBehaviour(SmartFluidTankBehaviour.TYPE)
                                        .allowInsertion();
                        IFluidHandler fh = be.getLevel().getCapability(Capabilities.FluidHandler.BLOCK,
                                        be.getBlockPos(), Direction.UP);
                        if (fh != null) {
                                fh.fill(gold, IFluidHandler.FluidAction.EXECUTE);
                        }
                });
                scene.idle(10);
                scene.overlay().showControls(drainTop, Pointing.RIGHT, 30)
                                .withItem(CMFluids.MOLTEN_GOLD.get().getFluidType().getBucket(gold));
                scene.idle(10);
        }
        ...
        public static void alloying(SceneBuilder builder, SceneBuildingUtil util) {
                ...
                scene.overlay().showControls(burnerSide, Pointing.LEFT, 30).rightClick()
                ...
                scene.idle(5);
                scene.overlay().showControls(basinSide, Pointing.RIGHT, 30)
                                .withItem(CMFluids.MOLTEN_IRON.get().getFluidType().getBucket(iron));
                scene.idle(40);
                ...
                scene.world().modifyBlockEntity(mixer, FoundryMixerBlockEntity.class,
                                FoundryMixerBlockEntity::startProcessingBasin);
                scene.idle(40);
                ...
                scene.idle(7);
                scene.world().setFilterData(util.select().position(1, 2, 1), SmartFluidPipeBlockEntity.class,
                                steelBucket);
                scene.idle(10);
