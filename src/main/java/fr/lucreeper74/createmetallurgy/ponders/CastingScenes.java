...
public class CastingScenes {
        public static void castingBlocks(SceneBuilder builder, SceneBuildingUtil util) {
                ...
                // Add Ingot Mold
                scene.overlay().showControls(tableTop, Pointing.LEFT, 30).rightClick()
                                .withItem(CMItems.GRAPHITE_INGOT_MOLD.asStack());
                scene.idle(7);
                scene.world().modifyBlockEntity(tablePos, CastingTableBlockEntity.class,
                                be -> be.moldInv.insertItem(0, CMItems.GRAPHITE_INGOT_MOLD.asStack(), false));
                scene.idle(20);
                ...
                scene.idle(40);
                scene.world().modifyBlockEntity(tablePos, CastingTableBlockEntity.class,
                                CastingTableBlockEntity::startProcess);
                scene.world().modifyBlockEntity(basinPos, CastingBasinBlockEntity.class,
                                CastingBasinBlockEntity::startProcess);
                ...
                scene.world().createItemEntity(util.vector().centerOf(basinFunnelPos), util.vector().of(0, 0, 0),
                                block);
                scene.world().modifyBlockEntity(basinPos, CastingBasinBlockEntity.class, be -> be.inv
                                .clearContent());
                scene.world().flapFunnel(tableFunnelPos, true);
                scene.world().createItemEntity(util.vector().centerOf(tableFunnelPos), util.vector().of(0, 0, 0),
                                ingot);
                scene.world().modifyBlockEntity(tablePos, CastingTableBlockEntity.class, be -> be.inv
                                .clearContent());
        }
