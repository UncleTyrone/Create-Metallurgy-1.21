...
public class CMCrushingRecipeGen extends CMProcessingRecipesGen {
    ...
    protected GeneratedRecipe rawOre(String metalName, Supplier<TagKey<Item>> input, Supplier<ItemLike> result,
            int xpMult) {
        return rawOre(metalName, input, result, false, xpMult);
    }

    protected GeneratedRecipe rawOreBlock(String metalName, Supplier<TagKey<Item>> input, Supplier<ItemLike> result,
            int xpMult) {
        return rawOre(metalName, input, result, true, xpMult);
    }

    protected GeneratedRecipe rawOre(String metalName, Supplier<TagKey<Item>> input, Supplier<ItemLike> result,
            boolean block, int xpMult) {
        return create("raw_" + metalName + (block ? "_block" : ""), b -> {
            int amount = block ? 9 : 1;
            ProcessingRecipeBuilder<?, ?, ?> builder = b.duration(400)
                    .require(input.get())
    ...
    public CMCrushingRecipeGen(PackOutput packOutput, CompletableFuture<HolderLookup.Provider> registries) {
