...
public class CastingRecipeGen extends CMRecipeProvider {
        ...
        ALL_COMPAT_METALS = allCompatMetals(),
                        ...
                        ANDESITE_ALLOY_FROM_IRON = basin("andesite_alloy_from_iron", Items.ANDESITE, true,
                                        CMFluids.MOLTEN_IRON, 90,
                                        AllBlocks.ANDESITE_ALLOY_BLOCK.get(), 360),
                        ANDESITE_ALLOY_FROM_ZINC = basin("andesite_alloy_from_zinc", Items.ANDESITE, true,
                                        CMFluids.MOLTEN_ZINC, 90,
                                        AllBlocks.ANDESITE_ALLOY_BLOCK.get(), 360),

                        COPPER_CASING = basinWithMoldTag(ItemTags.LOGS_THAT_BURN, true, CMFluids.MOLTEN_COPPER, 90,
                                        AllBlocks.COPPER_CASING.get(), 70),
                        BRASS_CASING = basinWithMoldTag(ItemTags.LOGS_THAT_BURN, true, CMFluids.MOLTEN_BRASS, 90,
                                        AllBlocks.BRASS_CASING.get(), 70),
        ...
        protected GeneratedRecipe allCompatMetals() {
                for (CMMetals metal : CMMetals.values()) {
                        ...
                        for (CMMetals.ItemType type : CMMetals.ItemType.values()) {
                                ...
                                CMRecipeTypes recipeType = type.equals(CMMetals.ItemType.BLOCK)
                                                ? CMRecipeTypes.CASTING_IN_BASIN
                                                : CMRecipeTypes.CASTING_IN_TABLE;
        ...
        protected GeneratedRecipe allStandard(CMMetals metal, Map<CMMetals.ItemType, ItemLike> typesItems) {
                for (CMMetals.ItemType type : CMMetals.ItemType.values()) {
                        ...
                        CMRecipeTypes recipeType = type.equals(CMMetals.ItemType.BLOCK) ? CMRecipeTypes.CASTING_IN_BASIN
                                        : CMRecipeTypes.CASTING_IN_TABLE;
         ...
         */
        protected GeneratedRecipe tableTag(String recipeId, ItemLike mold, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, TagKey<Item> resultTag, int duration) {
                return castingTagWithMold(CMRecipeTypes.CASTING_IN_TABLE, recipeId, mold, moldConsumed, fluid, amount,
                                resultTag, duration);
        }

        protected GeneratedRecipe tableTag(String recipeId, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount,
                        TagKey<Item> resultTag, int duration) {
                return castingTag(CMRecipeTypes.CASTING_IN_TABLE, recipeId, fluid, amount, resultTag, duration);
        }

        protected GeneratedRecipe basinTag(String recipeId, ItemLike mold, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, TagKey<Item> resultTag, int duration) {
                return castingTagWithMold(CMRecipeTypes.CASTING_IN_BASIN, recipeId, mold, moldConsumed, fluid, amount,
                                resultTag, duration);
        }

        protected GeneratedRecipe basinTag(String recipeId, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount,
                        TagKey<Item> resultTag, int duration) {
                return castingTag(CMRecipeTypes.CASTING_IN_BASIN, recipeId, fluid, amount, resultTag, duration);
        }

        protected GeneratedRecipe castingTagWithMold(CMRecipeTypes recipeType, String recipeId, ItemLike mold,
                        boolean moldConsumed, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount,
        ...
        protected GeneratedRecipe castingTag(CMRecipeTypes recipeType, String recipeId,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, TagKey<Item> resultTag, int duration) {
                ResourceLocation location = resultTag.location();
                create(recipeType, recipeId, b -> b.duration(duration)
                                .withCondition(new NotCondition(new TagEmptyCondition(location)))
         ...
         * @param duration Processing time
         */
        protected GeneratedRecipe table(String recipeId, ItemLike mold, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result, int duration) {
                return castingWithMold(CMRecipeTypes.CASTING_IN_TABLE, recipeId, mold, moldConsumed, fluid, amount,
                                result,
                                duration);
        }

        protected GeneratedRecipe table(String recipeId, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount,
                        ItemLike result, int duration) {
                return casting(CMRecipeTypes.CASTING_IN_TABLE, recipeId, fluid, amount, result, duration);
        }

        protected GeneratedRecipe basin(String recipeId, ItemLike mold, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result, int duration) {
                return castingWithMold(CMRecipeTypes.CASTING_IN_BASIN, recipeId, mold, moldConsumed, fluid, amount,
                                result,
                                duration);
        }

        protected GeneratedRecipe basin(String recipeId, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount,
                        ItemLike result, int duration) {
                return casting(CMRecipeTypes.CASTING_IN_BASIN, recipeId, fluid, amount, result, duration);
        }

        protected GeneratedRecipe castingWithMold(CMRecipeTypes recipeType, String recipeId, ItemLike mold,
                        boolean moldConsumed, FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result,
                        int duration) {
                create(recipeType, recipeId, b -> b.duration(duration)
                                .require(mold)
                                .require(fluid.get(), amount)
        ...
        protected GeneratedRecipe casting(CMRecipeTypes recipeType, String recipeId,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result, int duration) {
                create(recipeType, recipeId, b -> b.duration(duration)
                                .require(fluid.get(), amount)
                                .output(result));
        ...
        protected GeneratedRecipe tableWithMoldTag(TagKey<Item> moldTag, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result, int duration) {
        ...
        protected GeneratedRecipe basinWithMoldTag(TagKey<Item> moldTag, boolean moldConsumed,
                        FluidEntry<BaseFlowingFluid.Flowing> fluid, int amount, ItemLike result, int duration) {
                ResourceLocation location = moldTag.location();
        ...
        public CastingRecipeGen(PackOutput generator, CompletableFuture<HolderLookup.Provider> registries) {
         ...
         */
        protected GeneratedRecipe create(CMRecipeTypes type, ItemLike result,
                        UnaryOperator<CastingRecipeBuilder> transform) {
                GeneratedRecipe generatedRecipe = c -> {
                        CastingRecipeBuilder builder = transform.apply(new CastingRecipeBuilder(type,
                                        CreateMetallurgy.asResource(
                                                        BuiltInRegistries.ITEM.getKey(result.asItem()).getPath())));
                        builder.build(c, registries.join());
                };
         ...
         */
        protected GeneratedRecipe create(CMRecipeTypes type, String name,
                        UnaryOperator<CastingRecipeBuilder> transform) {
                GeneratedRecipe generatedRecipe = c -> {
                        CastingRecipeBuilder builder = transform
                                        .apply(new CastingRecipeBuilder(type, CreateMetallurgy.asResource(name)));
                        builder.build(c, registries.join());
                };
                all.add(generatedRecipe);
                return generatedRecipe;
