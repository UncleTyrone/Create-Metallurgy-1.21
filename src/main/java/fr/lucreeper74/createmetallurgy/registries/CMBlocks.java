...
import fr.lucreeper74.createmetallurgy.registries.CMDisplaySources;
...
public class CMBlocks {
        ...
        public static final BlockEntry<Block> WOLFRAMITE_ORE = REGISTRATE
                        ...
                        .loot((lt, b) -> {
                                ...
                                // Get the enchantment registry and create a holder reference
                                @SuppressWarnings("unchecked")
                                Registry<Enchantment> enchantmentRegistry = (Registry<Enchantment>) BuiltInRegistries.REGISTRY
                                                .get(Registries.ENCHANTMENT.location());
                                Holder<Enchantment> fortuneHolder = enchantmentRegistry
                                                .getHolderOrThrow(Enchantments.FORTUNE);
                                lt.add(b, lt.createSilkTouchDispatchTable(b,
                                                lt.applyExplosionDecay(b, LootItem
                                                                .lootTableItem(CMItems.RAW_WOLFRAMITE.get())
                                                                .apply(ApplyBonusCount
                                                                                .addOreBonusCount(fortuneHolder)))));
                        })
                        .transform(pickaxeOnly())
                        .tag(BlockTags.NEEDS_DIAMOND_TOOL)
        ...
        public static final BlockEntry<Block> COKE_BLOCK = REGISTRATE
                        ...
                        .tag(Tags.Blocks.STORAGE_BLOCKS)
                        .transform(TagGen.tagBlockAndItem(CMBlockTags.COKE_STORAGE_BLOCKS.tag,
                                        CMItemTags.COKE_STORAGE_BLOCKS.tag))
                        .tag(Tags.Items.STORAGE_BLOCKS)
                        .build()
                        .register();
        ...
        public static final BlockEntry<BeltGrinderBlock> BELT_GRINDER_BLOCK = REGISTRATE
                        ...
                        .onRegisterAfter(Registries.ITEM,
                                        v -> ItemDescription.useKey(v, "block.createmetallurgy.mechanical_grinder"))
                        .item()
                ...
                return REGISTRATE.block(colorName + "_light_bulb", p -> new LightBulbBlock(p, color))
                                                        ...
                                                        return ConfiguredModel.builder()
                                                                        .modelFile(p.models()
                                                                                        .withExistingParent(path
                                                                                                        ...
                                                                                                        + colorName,
                                                                                                        p.modLoc(path + "tube"))
                                                                                        .texture("0", p.modLoc(path
                                                                                                        + colorName)))
                                                                        .modelFile(p.models()
                                                                                        .withExistingParent(path
                                                                                                        + "tube_glow/"
                                                                                                        + colorName,
                                                                                                        p.modLoc(path + "tube_glow"))
                                                                                        .texture("0", p.modLoc(path
                                                                                                        + colorName)))
                                                                        .modelFile(p.models()
                                                                                        .withExistingParent(path
                                ...
                                .recipe((c, p) -> {
                                        ShapedRecipeBuilder.shaped(RecipeCategory.REDSTONE, c.get())
                                                        ...
                                                        .unlockedBy("has_tungsten_wire_spool", RegistrateRecipeProvider
                                                                        .has(CMItems.TUNGSTEN_WIRE_SPOOL.get()))
                                                        .save(p, CreateMetallurgy
                                                                        .asResource("crafting/" + c.getName()));
                                        ShapelessRecipeBuilder.shapeless(RecipeCategory.REDSTONE, c.get())
                                                        .requires(color.getTag())
                                                        .requires(CMItemTags.LIGHT_BULB.tag)
                                                        .unlockedBy("has_light_bulb",
                                                                        RegistrateRecipeProvider
                                                                                        .has(CMItemTags.LIGHT_BULB.tag))
                                                        .save(p, CreateMetallurgy.asResource("crafting/light_bulbs/"
                                                                        + c.getName() + "_from_other_light_bulb"));
                                })
                                .onRegisterAfter(Registries.ITEM,
                                                v -> ItemDescription.useKey(v, "block.createmetallurgy.light_bulb"))
                                .item(UncontainableBlockItem::new)
                                .tag(CMItemTags.LIGHT_BULB.tag)
                                .model((c, p) -> p
                                                .withExistingParent(colorName + "_light_bulb",
                                                                p.modLoc("block/light_bulb/item"))
        ...
        public static final BlockEntry<FaucetBlock> FAUCET_BLOCK = REGISTRATE
                        ...
                        .onRegisterAfter(Registries.ITEM,
                                        v -> ItemDescription.useKey(v, "block.createmetallurgy.faucet"))
                        .item()
